...
Fig1 = str_wrap("Fig 1: Data shown represents one second of activity captured at 128 Hz. Data is from six different subjects: three diagnosed with schizophrenic symptoms, and three determined to be non-symptomatic.", 120)
...
O2plot = ggplot(sample, aes(x=obs, y=O2, group=subject, color=diagnosis)) +
  ...
  scale_color_discrete(name = 'Diagnosis:', labels=c("non-symptomatic", "schizophrenic" ))+
  labs(title="O2 Region", caption=Fig1, y="voltage")
...
## Step 3) Full Data Box Plots

To explore the full dataset, we first gather the summary data of each sensor for each subject.

Note: This does not produce the final plot from our paper. Changes must not have been saved.
...
Minplot <- ggplot(allsum, aes(x=Min, group=diagnosis, fill=diagnosis)) +
  ...
  labs(title="Lowest Amp", x="voltage") +
  theme(axis.text.x=element_blank(), axis.title.x=element_blank())+
  coord_flip()
Maxplot <- ggplot(allsum, aes(x=Max, group=diagnosis, fill=diagnosis)) +
  geom_boxplot(show.legend = FALSE) +
  labs(title="Highest Amp", x=NULL) +
  theme(axis.text.x=element_blank(), axis.title.x=element_blank())+
  coord_flip()
Meanplot <- ggplot(allsum, aes(x=Mean, group=diagnosis, fill=diagnosis)) +
  geom_boxplot() +
  labs(title="Average Amp", x=NULL) +
  theme(axis.text.x=element_blank(), axis.title.x=element_blank())+
  coord_flip()+
  scale_fill_hue(name="Diagnosis", labels = c("non-symptomatic", "schizophrenic"))

grid.arrange(Minplot, Maxplot, Meanplot, nrow=1, widths=c(.75,.75,1.5))
...
This plot was not saved either.