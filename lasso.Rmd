...
for ( imp in seq(10) ) {
  ...
  # Dumm code categorical predictor variables
  ...
  aucs[imp,] = glmnet:::auc(test.data$diagnosis,probabilities)
...
title = sprintf("Features Chosen at least %d time(s)", freqPercent)
ggplot(nonzeroImportance, aes(x=reorder(name, frequency), weight=frequency)) + 
  geom_bar() +
